...
public class ContactDetailActivity extends BaseActivity
    ...
    static class UpdateContact extends AsyncTask<Uri, Void, Void> {
        ...
        protected Void doInBackground(Uri... args) {
            Uri uri = args[0];
            if (null != uri) {
                // if uri is not null, this is updating existing entry
                resolver.update(uri, vals, null, null);
            } else {
                // if the uri is null, this is creating new entry
                resolver.insert(ContactsContract.URI, vals);
            }
            return null;
        }
    }
    ...
    protected void onCreate(Bundle state) {
        ...
        if (null != uri) {
            // get the uri if it is modifying existing entry
    ...
    /**
     * The update method makes a sanity check
     * (to make sure that the user hasn’t mistakenly submitted an empty form)
     * and then creates a ContentValues object to hold the user’s input.
     * Again, note that there is no intermediate POJO (Plain Ole Java Object)
     * representation of the contact. Instead, the UI represents that data as
     * an instance of the standard, pervasive ContentValues object.
     */
    void update() {
        ...
        // User input has been put into vals
    ...
    /**
     * Add User input String into ContentValue
     * @param view - The View of input
     * @param oldVal - The old value
     * @param vals   - The ContentValue
