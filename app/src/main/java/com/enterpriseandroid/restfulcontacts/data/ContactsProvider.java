...
public class ContactsProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues vals) {
        ...
        // When the RESTService.insert method returns, having scheduled the remote transaction,
        // the insert method completes its work by calling localInsert.
        // localInsert adds the newly created record to the database,
        // including the “dirty” lag, marking it as unsynchronized, and the transaction ID,
        // identifying the transaction that has been scheduled to synchronize it.
    ...
    public Uri localInsert(Uri uri, ContentValues vals) {
        ...
        if (0 > pk) { uri = null; }
            ...
            //The localInsert method also notiies any observers that the dataset
            // backing the content provider’s URI has changed. For example, if
            // the user is now looking at the list of all contacts — quite likely,
            // since creating a new contact causes the UI to navigate back to the list view
            // — that list displays the newly entered, but out of sync, contact.
            // Because it is out of sync but has a transaction scheduled to synchronize it,
            // the status bar at the left of the row should be yellow.
            // The notiication generated in the localInsert method will cause the UI
            // to update to show the yellow status bar.
